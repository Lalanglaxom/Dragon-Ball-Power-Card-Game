...
effect_file_path = "Power Card Game - Digital - Effect.csv"
...
# Read data from the CSV file
header_row, csv_data = read_csv(effect_file_path)

# Function to update the file paths with a new folder file path
def update_effect_file_paths(csv_data, new_folder_path, big_image_path):
    for row in csv_data:
        # Loop through each file in the folder
        for filename in os.listdir(big_image_path):
            # Get the full path of the file
...
# Update the file paths with the new folder file path
update_effect_file_paths(
    csv_data,
    r"D:/Godot Game/Dragon-Ball-Power-Card-Game/card/effect/image/160x240/",
    r"D:/Godot Game/Dragon-Ball-Power-Card-Game/card/effect/image/742x1122/",
)


# Write the modified data back to the CSV file
with open(effect_file_path, "w", newline="", encoding="utf-8") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(header_row)
    writer.writerows(csv_data)
...
json_data_effect_path = "card/effect/effect_database.json"
json_collection_effect_path = "card/effect/effect_collection.json"
...
for index, row in enumerate(csv_data):
    ...
    card_data = {
        ...
        "effect": effect.strip(),
        "front_mini_path": mini_image.strip(),
        "back_mini_path": "res://card/effect/image/160x240/back.png",
        "front_image_path": texture_path.strip(),
        "back_image_path": "res://card/effect/image/742x1122/back.png",
        "resource_script_path": "res://card/script/effect_data.gd",
    }
...
# Write JSON data to the file
with open(json_data_effect_path, "w", encoding="utf-8") as jsonfile:
    json.dump(json_data, jsonfile, ensure_ascii=False, indent=2)

with open(json_collection_effect_path, "w", encoding="utf-8") as jsonfile:
    json.dump(json_collection, jsonfile, indent=2)

print("JSON data has been written to", json_data_effect_path)
